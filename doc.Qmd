...
Considerando le variabili prec, popn, educ, hous, nonw, poor e log(nox), che, dall'analisi bivariata sono quelle che sembrano essere più correlate alla variabile mort, creiamo il nostro primo modelllo.

```{r}
mod1 <- lm(mort ~ prec + popn + educ + hous + nonw + poor + log(nox), data = Pollution)
summary(mod1)
AIC(mod1)                        
```

Si nota che questo modello ha un Adjusted R-squared di 0.6752 e l'AIC di 607.8757
