...
L'AIC risultante è di 429.63, con variabili considerate finali $$\text{mort} \sim  \beta_0+ \beta_1\cdot \text{prec} + \beta_2\cdot \text{jant} + \beta_3\cdot\text{jult} + \beta_4\cdot\text{ovr65} + \beta_5\cdot\text{popn} + \beta_6\cdot \text{educ} + \beta_7\cdot \text{nonw} + \beta_8\cdot \text{hc} + \beta_9\cdot\text{nox}$$
...
$$ \text{mort} \sim \beta_0 + \beta_1\cdot \text{prec} + \beta_2\cdot\text{educ}+ \beta_3\cdot\text{hous}+\beta_4\cdot\text{nonw}+\beta_5\cdot\text{poor}+\beta_6\cdot \log(\text{nox}) + \beta_7\cdot\text{educ}*\text{hous} +  \beta_8\cdot\text{nonw}*\text{poor} +  \beta_9\cdot\text{educ}*\text{poor} +  \beta_{10}\cdot\text{educ}*\text{nonw}+\beta_{11}\cdot\text{educ}*\text{nonw}*\text{poor} $$
...
#### Secondo modello StepAIC senza 37
...
| Secondo Modello StepAIC senza 37 |    80.65%    | 562.33 |
...
In generale, una combinazione di entrambi i criteri è spesso considerata. Se si desidera un equilibrio tra un buon $R^2$ aggiustato e un basso AIC, si potrebbe considerare il Secondo Modello StepAIC senza 37, che ha un $R^2$ buon aggiustato (80.65%) e un AIC relativamente basso (562.33).

Ovviamente si è consci che cìè un aumento della complessità del modello tra il primo primo proposto. Si vede anche che non è proporzionale la crescita dell' $R^2$ e dell'AIC. Per quanto dunque il modello trovato spieghi le variabili si può supporre anche che possa essere overfittato (un grande aumento dell'AIC deriva probabilmente dall'avere incluso molti parametri aggiuntivi), ovvero, spiega bene il presente dataset ma potrebbe non generalizzare bene nuovi dati,
